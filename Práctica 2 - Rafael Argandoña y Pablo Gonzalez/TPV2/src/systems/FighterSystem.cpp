...
void FighterSystem::initSystem() {
	...
	mngr->addComponent<Transform>(f, PLAYER_INITIAL_POS, Vector2D(), PLAYER_WIDTH, PLAYER_HEIGHT);
	mngr->addComponent<FighterCtrl>(f, &sdlutils().soundEffects().at(THRUST));
	mngr->addComponent<DeAcceleration>(f);
	mngr->addComponent<ShowAtOppositeSide>(f);
	mngr->addComponent<Gun>(f, &sdlutils().soundEffects().at(FIRESFX));
